...
//	std::map<std::string, TH1*>::iterator it;
	...
	for (std::map<std::string, TH1*>::iterator it = _hitmapMap.begin(); it != _hitmapMap.end(); ++it) {
...
void OnlineMonWindow::registerGraph(std::string tree,TGraph* g, std::string op, const unsigned int l) {
	if (g==NULL)// check if valid histogram
	{
